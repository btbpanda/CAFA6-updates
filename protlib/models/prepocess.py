...
def get_folds(seq, n_folds=5, random_state=42):
    folds = GroupKFold(n_folds, shuffle=True, random_state=random_state)
    res = np.zeros(len(seq), dtype=np.int32)

    for n, (f0, f1) in enumerate(folds.split(seq, seq, seq)):
        res[f1] = n

    return res


def get_tax_ohe(tax_id, tax_list, intercept=True):
    tax_list = np.asarray(tax_list)

    if type(tax_id) is pd.Series:
        tax_id = tax_id.values

    tax = tax_id[:, None] == tax_list[None, :]

    if intercept:
        tax = np.concatenate([
            tax.sum(axis=1, keepdims=True) == 0, tax
        ], axis=1)

    return tax.astype(np.float32)
...
def get_features(data, embed_path, prefix, embed_list, tax_list):
    ...
    for embed in embed_list:
        res.append(
            np.load(os.path.join(embed_path, embed, f'{prefix}_embeds.npy'))
        )

    return np.concatenate(res, axis=1, dtype=np.float32)
#
#
# def get_tax(fasta):
#     tax_list = [
...
def get_targets_from_parquet(path, ontologies, split, ids=None, names=None, fillna=False):
    ...
    for i in range(3):
        ...
        trg = pd.read_parquet(
            sorted(glob.glob(os.path.join(path, G.namespace, '*.parquet'))),
            columns=names_
        )
