...
if __name__ == '__main__':
    ...
    train_elabels = sorted(glob.glob(os.path.join(args.elabels_path, 'train_*.tsv')))
    old_train_elabels = sorted(glob.glob(os.path.join(args.elabels_path, 'old_train_*.tsv')))

    goa_data = [
        get_labels(
            path=x, G=G, idx=train['EntryID']
        ) + get_labels(
            path=y, G=G, idx=train['EntryID']
        ) for (x, y) in zip(train_elabels, old_train_elabels)
    ]
    ...
    test_elabels = sorted(glob.glob(os.path.join(args.elabels_path, 'test_*.tsv')))

    test_goa_data = [
        get_labels(
            path=x, G=G, idx=ids_to_take
        ) for x in test_elabels
    ]
    ...
    model = GCNStacker(
        len(config['models']), len(test_elabels), G,
